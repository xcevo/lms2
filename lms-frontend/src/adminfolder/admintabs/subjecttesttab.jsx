...
import SubjectTestPreview from "../adminModels/subjectTestpreview";
import API_BASE_URL from "../../../config";

// Subject-Test base
const ENDPOINT = `${API_BASE_URL}/api/admin/subject-test`;

export default function SubjectTestTab() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err?.message || `Failed to load subject tests (HTTP ${res.status})`);
      }
      ...
      console.error(e);
      alert(e.message || "Failed to load subject tests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const uploadTest = async () => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch(`${ENDPOINT}/upload-subtest`, {
        method: "POST",
        headers: authHeaders,
        body: fd,
      });
  ...
  const deleteOne = async (t) => {
    if (!confirm(`Delete subject test "${t.title}"?`)) return;
    try {
      const res = await fetch(`${ENDPOINT}/deletesubtest/${encodeURIComponent(t._id)}`, {
        method: "DELETE",
        headers: authHeaders,
      });
      const data = await res.json();
  ...
  const openPreview = async (t) => {
    try {
      const res = await fetch(`${ENDPOINT}/previewsubtest/${encodeURIComponent(t._id)}`, {
        headers: { ...authHeaders, Accept: "application/json" },
        cache: "no-store",
      });
      const data = await res.json();
  ...
  const downloadFile = async (t) => {
    try {
      const res = await fetch(`${ENDPOINT}/downloadsubtest/${encodeURIComponent(t._id)}`, {
        headers: authHeaders,
        cache: "no-store",
      });
      if (!res.ok) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <input
            ...
            className="hidden"
            id="subjectTestFileInput"
          />
          <label
            htmlFor="subjectTestFileInput"
