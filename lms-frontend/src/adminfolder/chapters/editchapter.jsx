...
export default function EditChapterModal({ open, onClose, onSaved, subjectId, chapter }) {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/admin-subject/${encodeURIComponent(
          subjectId
        )}/edit-chapter/${encodeURIComponent(chapter._id)}`,
  ...
  return (
    <>
  <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[55]" />
  <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
        ...
        >
          <div className="px-5 py-3 border-b border-white/10 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-green-400">Edit Chapter</h3>
          </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm mb-1">Replace PDF (optional)</label>
                <input
                  type="file"
              ...
              <div>
                <label className="block text-sm mb-1">Replace Video (optional)</label>
                <input
                  type="file"
                  accept="video/*"
