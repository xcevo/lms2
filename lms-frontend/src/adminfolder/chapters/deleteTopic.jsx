...
/**
 * Confirmation modal for deleting a topic
 * Props:
 * - open: boolean
 * - topicName?: string
 * - busy?: boolean
...
export default function DeleteTopicModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" aria-modal="true" role="dialog">
      {/* overlay with blur (matches rest of app) */}
      ...
      <div className="relative z-10 w-[min(420px,94vw)] rounded-lg border-[0.3px] border-emerald-800 bg-white/5 shadow-xl">
        <div className="px-5 py-4 border-b border-white/10">
          <h3 className="text-lg font-semibold text-slate-100">Delete topic?</h3>
        </div>
        ...
        <div className="px-5 py-4 text-slate-300">
          <p className="leading-relaxed">
            ...
            {topicName ? (
              <>
                {" "}
                <span className="font-semibold text-emerald-500">“{topicName}”</span>
              </>
            ) : null}
            {" "}from this chapter.
          </p>
